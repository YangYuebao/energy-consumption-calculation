<template>
  <n-divider title-placement="left"> 预输入参数 </n-divider>
  <n-space vertical>
  ...
  <n-divider title-placement="left"> 空压机排气压力计算 </n-divider>
  <n-space vertical>
    <n-grid x-gap="5" :cols="10">
      <n-gi :span="2">
    ...
    <n-grid v-if="pcp_option_id === '1'" x-gap="5" :cols="10">
      <n-gi :span="2">
    ...
    <n-grid v-if="pcp_option_id === '2'" x-gap="5" :cols="10">
      <n-gi :span="2">
  ...
  <n-divider title-placement="left"> 压缩机功耗计算 </n-divider>
  <n-space vertical>
    <n-grid x-gap="5" :cols="10">
      <n-gi :span="2">
      ...
      <n-gi :span="6">
        <n-dynamic-input v-model:value="Pcp" :min="ncp" :max="ncp">
          <template #default="{ value }">
            <div style="display: flex; align-items: center; width: 100%">
              <n-input-number v-model:value="value[0]" style="margin-right: 12px; width: 450px" :show-button="false">
                <template #suffix> 运行功率 kW </template>
              </n-input-number>
            </div>
          </template>
        </n-dynamic-input>
    ...
    <n-grid v-if="Pcp_option_id === '2' && Qcp_option_id === '1'" x-gap="5" :cols="10">
      <n-gi :span="2">
      ...
      <n-gi :span="6">
        <n-dynamic-input v-model:value="Qcp" :min="ncp" :max="ncp">
          <template #default="{ value }">
            <div style="display: flex; align-items: center; width: 100%">
              <n-input-number v-model:value="value[0]" style="margin-right: 12px; width: 450px" :show-button="false">
                <template #suffix> 吸气状态 m³/min </template>
              </n-input-number>
            </div>
          </template>
        </n-dynamic-input>
    ...
    <n-grid v-if="Pcp_option_id === '2' && Qcp_option_id === '2'" x-gap="5" :cols="10">
      ...
      <n-gi :span="6">
        <n-input-number v-model:value="ncp_st" :show-button="false">
          <template #suffix> 下列额定参数先输入工频机 </template>
        </n-input-number>
      </n-gi>
    </n-grid>
  ...
  <n-divider title-placement="left"> 空压站其余设备功耗 </n-divider>
  <n-space vertical>
    <n-grid x-gap="5" :cols="10">
      <n-gi :span="2">
  ...
  <n-divider title-placement="left"> 空压站供气有效能计算 </n-divider>
  <n-space vertical>
    <n-grid x-gap="5" :cols="10">
      <n-gi :span="2">
...
function supply_E_cal() {
  if (supply.value.length === 1) {
    ...
    eta_total.value =
      ((1 + 0.2 * eta_R.value) * E_supply.value) / (totalPcp.value + Pdryer.value + Pother.value + Pcooler.value);
  } else {
    E_supply.value = 0;
    eta_total.value = 0;
    for (var i = 0; i < supply.value.length; i++) {
...
function excelExport() {
  ...
  worksheet['!cols'] = [{ wpx: 170 }, { wpx: 60 }, { wpx: 100 }, { wpx: 100 }, { wpx: 120 }, { wpx: 100 }]; // 设置每列的列宽，10代表10个字符，注意中文占2个字符
...
const t1 = [
  [55.0, 50.0, 45.5, 41.5, 38.0],
  [51.5, 47.0, 43.0, 39.0, 35.5],
  [58.0, 53.0, 48.0, 44.0, 40.0],
  [54.5, 50.0, 45.0, 41.5, 37.5],
  [61.0, 55.5, 50.5, 46.0, 42.0],
  [57.5, 52.5, 47.5, 43.5, 39.5],
  [64.0, 58.0, 53.0, 48.5, 44.0],
  [60.0, 55.0, 50.0, 45.5, 41.5]
];
const t2 = [
  [52.5, 47.5, 43.5, 39.5, 36.5],
  [49.0, 45.0, 41.0, 37.0, 34.0],
  [55.0, 50.5, 45.5, 42.0, 38.0],
  [52.0, 47.5, 43.0, 39.5, 36.0],
  [58.0, 53.0, 48.0, 44.0, 40.0],
  [55.0, 50.0, 45.0, 41.5, 37.5],
  [61.0, 55.0, 50.5, 46.0, 42.0],
  [57.0, 52.5, 47.5, 43.5, 39.5]
];
const t3 = [
  [50.0, 45.5, 41.5, 37.5, 34.5],
  [47.0, 42.5, 39.0, 35.5, 32.5],
  [52.5, 48.0, 43.5, 40.0, 36.5],
  [49.5, 45.5, 41.0, 37.5, 34.0],
  [55.5, 50.5, 46.0, 42.0, 38.0],
  [52.5, 47.5, 43.0, 39.5, 36.0],
  [58.0, 52.5, 48.0, 44.0, 40.0],
  [54.5, 50.0, 45.5, 41.5, 37.5]
];
const t4 = [
  [48.0, 43.5, 39.5, 36.0, 33.0],
  [45.0, 41.0, 37.5, 34.0, 31.0],
  [50.5, 46.0, 41.5, 38.5, 35.0],
  [47.5, 43.5, 39.0, 36.0, 32.5],
  [53.0, 48.5, 44.0, 40.0, 36.5],
  [50.0, 45.5, 41.5, 38.0, 34.5],
  [55.5, 50.5, 46.0, 42.0, 38.5],
  [52.0, 48.0, 43.5, 39.5, 36.0]
];
const t5 = [
